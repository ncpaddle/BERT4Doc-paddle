    ...
    def _create_examples(self, lines, set_type, data_num=None):
        ...
        for (i, line) in enumerate(lines):
            ...
            if len(s) > 510:
                s = s[: 255] + s[-255:]
            guid = "%s-%s" % (set_type, i)
            text_a = convert_to_unicode(" ".join(s))
            label = convert_to_unicode(str(line[0]))
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
        return examples
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            for j in range(scale):
                ...
                text_a = convert_to_unicode(" ".join(s[:r]))
                text_b = convert_to_unicode(" ".join(s[r:]))
                label = convert_to_unicode(str(line[0]))
                if i%1000==0:
                    print(i)
                    print("guid=",guid)
...
def getArgs():
    ...
    parser.add_argument("--model_name_or_path",
                        default="../../furthered_imdb_pretrained",
                        type=str,
...
def train(args, model, tokenizer, processor, label_list, n_gpu):
    ...
    if args.discr:
        ...
        optimizer_parameters = [
            ...
            {'params': [p for n, p in model.named_parameters() if
             ...
             'weight_decay_rate': 0.01, 'lr': args.learning_rate / 2.6},
            {'params': [p for n, p in model.named_parameters() if
    ...
    optimizer = AdamW(parameters=model.parameters(),
                      apply_decay_param_fun=lambda x: x in optimizer_parameters,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                #     'label_ids': label_ids
                # }
                # paddle.save(inputs, 'fake_data_paddle.bin')
                # assert 1 == 2
