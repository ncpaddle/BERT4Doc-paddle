...
class BertForSequenceClassification(BertPretrainedModel):
    ...
    def forward(self,
        ...
        if len(self.layers) > 0:
            ...
            hidden_state = paddle.concat(hidden_state, axis=1)  # shape: bs, layers_num, 768

            if self.pooling == 'max':
                hidden_state, _ = paddle.max(hidden_state, axis=1)
            elif self.pooling == 'mean':
                hidden_state = paddle.mean(hidden_state, axis=1)
            else:
